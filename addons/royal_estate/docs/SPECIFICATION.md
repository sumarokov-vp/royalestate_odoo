...
**Важно:** ERP работает с CRM (сделками), а не с объектами. Royal Estate — источник данных об объектах, CRM — источник данных для расчёта зарплат.
...
### Участники для расчёта комиссии

В объекте фиксируем, кто участвовал в работе:

| Поле | Описание |
|------|----------|
| `listing_coordinator_id` | Кто внёс в базу |
| `listing_agent_id` | Листинг-агент |

В сделке (CRM) фиксируем:

| Поле | Описание |
|------|----------|
| `isa_user_id` | Кто квалифицировал лид |
| `buyer_agent_id` | Агент покупателя |
| `transaction_coordinator_id` | Координатор сделки |

Подробнее: [INTEGRATIONS.md](./INTEGRATIONS.md#2-отслеживание-участников-для-расчёта-комиссии)
